...
function getDecoratorEnv() {
  return process.env.NEXT_PUBLIC_DECORATOR_ENV ?? (process.env.NAIS_CLUSTER_NAME === "prod-gcp" ? "prod" : "dev");
}
...
app.get("/hent-dokument/:dokumentType/:dokumentId.pdf", async (req, res) => {
  ...
  res.setHeader("Content-Disposition", `inline; filename="${dokumentType}-${dokumentId}.pdf"`);
...
app.use("/feilmelding", (_req, res) => renderDecoratedPage(res, "dist/feilmelding/index.html"));
app.use("/404", (_req, res) => renderDecoratedPage(res, "dist/404/index.html", 404));
app.use("/403", (_req, res) => renderDecoratedPage(res, "dist/403/index.html", 403));
app.use("/ugyldig", (_req, res) => renderDecoratedPage(res, "dist/ugyldig/index.html", 400));
