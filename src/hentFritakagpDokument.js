...
const FRITAKAGP_TYPE = new Map([
  [
    "gravid-soeknad",
    {
      apiPath: "/api/v1/gravid/soeknad",
      pdfgenPath: "/api/v1/genpdf/fritakagp/gravid-soknad",
    },
  ],
  [
    "gravid-krav",
  ...
  [
    "kronisk-soeknad",
    {
      apiPath: "/api/v1/kronisk/soeknad",
      pdfgenPath: "/api/v1/genpdf/fritakagp/kronisk-soknad",
    },
  ],
  [
    "kronisk-krav",
...
export async function hentFritakagpDokument(token, dokumentType, dokumentId) {
  ...
  const jsonResponse = await fetch(fritakagpUrl, {
    method: "GET",
    headers: {
